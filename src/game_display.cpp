...
game_display* game_display::create_dummy_display(CVideo& video)
{
	...
	static reports rep_;
	return new game_display(dummy_board, video, std::shared_ptr<wb::manager>(), rep_,
...
{
	const time_of_day& tod = get_tod_man().get_time_of_day();

	if( !first_turn_) {
		const time_of_day& old_tod = get_tod_man().get_previous_time_of_day();
...
{
	return get_tod_man().get_time_of_day(loc);
}

bool game_display::has_time_area() const
{
	return get_tod_man().has_time_area();
}
