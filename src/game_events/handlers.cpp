...
std::vector<std::string> event_handler::names(const variable_set* vars) const
{
	...
	if(vars && utils::might_contain_variables(names)) {
		names = utils::interpolate_variables_into_string(names, *vars);
	}

	// Do some standardization on the name field.
	// Split the name field...
	std::vector<std::string> standardized_names = utils::split(names);
	...
	if(!names.empty()) {
		...
		for(std::string& single_name : standardized_names) {
			if(utils::might_contain_variables(single_name) && !vars) {
	...
	standardized_names.erase(std::remove_if(standardized_names.begin(), standardized_names.end(), std::mem_fn(&std::string::empty)), standardized_names.end());
...
{
	for(const auto& filter : cfg.all_children_range()) {
